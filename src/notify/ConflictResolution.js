...
class ConflictResolution {
  constructor({ strategy = 'ps', logger }) {
    ...
    this.logger = logger;
    this.logger.debug({
      message: `conflict resolution strategy "${strategy}"`,
    });
  }
  ...
  select(actions) {
    ...
    // conflict resolution
    this.logger.debug({
      message: `conflict resolution starting with ${actions.length}`,
    });
  ...
  resolveByPriority(actions) {
    ...
    const selected = actions.filter(
      (action) => action.priority === highestPrio,
    );
    this.logger.debug({
  ...
  resolveBySpecificity(actions) {
    const isMoreSpecific = (action, rhs) =>
      ...
      intersection(action.premises, rhs.premises).length ===
        rhs.premises.length;
