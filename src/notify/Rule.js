...
class Rule {
  ...
  assert() {
    ...
    assert(isString(this.name), '"name" must be a string');
    assert(this.when.length, '"when" is required with at least one premise');
    assert(
      this.when.reduce((acc, premise) => acc && isFunction(premise), true),
      '"when" must be a function or an array of functions',
    );
    assert(this.then, '"then" is required');
    assert(isFunction(this.then), '"then" must be a function');
    assert(isInteger(this.priority), '"priority" must be an integer');
    assert(isBoolean(this.final), '"final" must be a boolean');
    assert(
